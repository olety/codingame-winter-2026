...
/* Conv3x3 int4 — skip zero input channels (for conv1 with binary grid) */
...
/* Standard conv for conv2/conv3 (dense activations) */
...
static void global_avg_pool(const float* __restrict__ in, float* out, int ch) {
    float inv = 1.0f / HW;
    for (int c = 0; c < ch; c++) {
        float s = 0; const float *src = &in[c * HW];
...
static void linear_f(const float* __restrict__ wt, const float* __restrict__ bi,
    ...
    for (int o = 0; o < out_f; o++) {
        float s = bi[o]; const float *wr = &wt[o * in_f];
...
static void nn_forward_skipzero(int ch) {
    conv3x3_skipzero(wt1, sc1, bi1, IN_CH, ch, grid, buf_a);
...
int main() {
    ...
    for (int s = 0; s < 4; s++) {
        ...
        encode_features(); nn_forward_skipzero(ch); /* warmup */
        struct timespec t0, t1;
        clock_gettime(CLOCK_MONOTONIC, &t0);
        for (int i = 0; i < ni; i++) { encode_features(); nn_forward_skipzero(ch); }
        clock_gettime(CLOCK_MONOTONIC, &t1);
        double e = (t1.tv_sec-t0.tv_sec) + (t1.tv_nsec-t0.tv_nsec)/1e9;
        double ms = e / ni * 1000;
        fprintf(stderr, "%dch skipzero: %.2fms/eval  (%d in 40ms, %d in 850ms)\n",
