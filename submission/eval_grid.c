...
#define IN_CH 19
#define SCALARS 6
#define POLICY_OUT 20
#define MAX_CH 96

static float grid_in[IN_CH * HW];
static float scalar_feat[SCALARS];
static float buf_a[MAX_CH * HW], buf_b[MAX_CH * HW];
static float pool[MAX_CH + SCALARS];
static float policy_logits[POLICY_OUT], policy_probs[POLICY_OUT];
...
static void conv3x3_skipzero(const int8_t* __restrict__ wt, const float* __restrict__ sc,
                              const float* __restrict__ bi, int ic, int oc,
                              const float* __restrict__ in, float* __restrict__ out) {
    int active[MAX_CH], na = 0;
    for (int c = 0; c < ic; c++) {
        int ib = c*HW, any = 0;
        for (int i = 0; i < HW; i++) { if (in[ib+i] != 0.0f) { any=1; break; } }
        if (any) active[na++] = c;
    }
    for (int o=0;o<oc;o++){float bv=bi[o];int b=o*HW;for(int i=0;i<HW;i++)out[b+i]=bv;}
    for (int o=0;o<oc;o++){
        float s=sc[o];int ob=o*HW;
        for(int ai=0;ai<na;ai++){int c=active[ai];
            ...
            for(int edge=0;edge<2;edge++){int y=edge==0?0:H-1;
                float ws[9]={w0,w1,w2,w3,w4,w5,w6,w7,w8};
                for(int x=0;x<W;x++){float a=0;
                for(int ky=0;ky<3;ky++){int iy=y+ky-1;if(iy<0||iy>=H)continue;
                for(int kx=0;kx<3;kx++){int ix=x+kx-1;if(ix<0||ix>=W)continue;
                a+=in[ib+iy*W+ix]*ws[ky*3+kx];}}out[ob+y*W+x]+=a;}}
            for(int y=1;y<H-1;y++){float ws[9]={w0,w1,w2,w3,w4,w5,w6,w7,w8};
                for(int xi=0;xi<2;xi++){int x=xi==0?0:W-1;float a=0;
...
static void global_avg_pool(const float* __restrict__ in, float* out, int ch) {
    float inv=1.0f/HW;
    for(int c=0;c<ch;c++){float s=0;const float*src=&in[c*HW];for(int i=0;i<HW;i++)s+=src[i];out[c]=s*inv;}
}

static void linear_f(const float* __restrict__ wt, const float* __restrict__ bi,
                     const float* __restrict__ in, float* __restrict__ out, int in_f, int out_f) {
    for(int o=0;o<out_f;o++){float s=bi[o];const float*wr=&wt[o*in_f];for(int i=0;i<in_f;i++)s+=wr[i]*in[i];out[o]=s;}
}
...
static void nn_forward(int ch) {
    conv3x3_skipzero(wt1,sc1,bi1,IN_CH,ch,grid_in,buf_a);
    ...
    int feat=ch+SCALARS;
    linear_f(pw,pb,pool,policy_logits,feat,POLICY_OUT);
    memcpy(policy_probs,policy_logits,POLICY_OUT*sizeof(float));
    softmax(policy_probs,POLICY_OUT);
    float v;linear_f(vw,&vb,pool,&v,feat,1);
    value_out=tanhf(v);
}

static void encode_features(void) {
    memset(grid_in,0,sizeof(grid_in));
    for(int c=0;c<8;c++)for(int i=0;i<60;i++)grid_in[c*HW+(i*17+c*7)%HW]=1.0f;
...
int main() {
    ...
    int sizes[] = {8, 12, 16, 24, 32, 48, 64, 96};
    int iters[] = {80, 60, 40, 25, 20, 10, 6, 3};
    int nsizes = 8;

    for(int s=0;s<nsizes;s++){
        int ch=sizes[s],ni=iters[s];
        encode_features();nn_forward(ch);
        struct timespec t0,t1;
        clock_gettime(CLOCK_MONOTONIC,&t0);
        for(int i=0;i<ni;i++){encode_features();nn_forward(ch);}
        clock_gettime(CLOCK_MONOTONIC,&t1);
        double e=(t1.tv_sec-t0.tv_sec)+(t1.tv_nsec-t0.tv_nsec)/1e9;
        double ms=e/ni*1000;
        fprintf(stderr,"%3dch: %5.2fms  %2d/40ms  %3d/850ms\n",
                ch,ms,(int)(40.0/ms),(int)(850.0/ms));
        fflush(stderr);
    }
    printf("WAIT\n");
}
