...
/* SKIP ZERO: same as baseline but skip input channels where all pixels are zero.
   Grid channels are binary (0/1). Many channels are mostly/all zero for a given position.
   Only helps conv1 (19 input channels, binary). Conv2/3 have dense activations after ReLU. */
static void conv_skipzero(const int8_t* __restrict__ wt, const float* __restrict__ sc,
    ...
    /* Precompute which input channels have any nonzero values */
    int active[96]; int n_active = 0;
    for (int c = 0; c < ic; c++) {
        int ib = c * hw;
        int has_nonzero = 0;
        for (int i = 0; i < hw; i++) { if (in[ib+i] != 0.0f) { has_nonzero = 1; break; } }
        if (has_nonzero) active[n_active++] = c;
    }
...
/* Baseline conv for conv2/conv3 (dense activations, skip-zero won't help) */
...
int main() {
    ...
    /* Simulate realistic binary grid: ~8 of 19 channels active */
    memset(b1, 0, sizeof(b1));
    for(int c=0;c<8;c++) for(int i=0;i<H*W;i++) b1[c*H*W+i] = ((i*7+c*13)%3==0) ? 1.0f : 0.0f;
    ...
    for(int i=0;i<3;i++){conv_skipzero(wt1,sc,bi,IN,96,b1,b2);conv(wt2,sc,bi,96,96,b2,b1);conv(wt3,sc,bi,96,96,b1,b2);}
    clock_gettime(CLOCK_MONOTONIC,&t1);
    double e=(t1.tv_sec-t0.tv_sec)+(t1.tv_nsec-t0.tv_nsec)/1e9;
    fprintf(stderr,"SKIPZERO 96ch: %.2f ms/inf (3 iters, ~8/19 active ch)\n",e/3*1000); fflush(stderr);
    printf("WAIT\n");
}
