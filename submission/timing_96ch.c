...
int main(){
 ...
 for(int i=0;i<3;i++){conv_int4(wt1,sc,bi,IN,CH,b1,b2);conv_int4(wt2,sc,bi,CH,CH,b2,b1);conv_int4(wt3,sc,bi,CH,CH,b1,b2);}
 clock_gettime(CLOCK_MONOTONIC,&t1);
 double e=(t1.tv_sec-t0.tv_sec)+(t1.tv_nsec-t0.tv_nsec)/1e9;
 fprintf(stderr,"%dch/3L int4: %.2f ms/inf (3 iters)\n",CH,e/3*1000);
 printf("WAIT\n");
}
