...
static double danger_score(const State* s, int owner) {
    ...
    for (int i = 0; i < s->nbirds; i++) {
        ...
        double penalty = 0;
        if (ttw <= 1) penalty = -200.0;
        else if (ttw <= 2) penalty = -80.0;
        else if (ttw <= 3) penalty = -20.0;
...
static double deepen_branch_c(const State* s, int owner, double deadline_ms) {
    ...
    /* Order opp actions (worst for me first) */
...
static SearchResult choose_action_maximin(const State* s, int owner, double deadline_ms) {
    ...
    /* Phase 3: Order my actions by 1-ply eval vs default opp */
    double my_prior[MAX_ACTIONS];
    int my_order[MAX_ACTIONS];
    for (int a = 0; a < nmy; a++) {
        State nx = sim_state(s, owner, &my_acts[a], &opp_acts[0]);
        my_prior[a] = evaluate(&nx, owner);
        my_order[a] = a;
    }
    ...
    typedef struct { int idx; double worst; double mean; } RootEntry;
    ...
    /* Phase 6: Selective deepening of top actions */
    int deep_count = DEEPEN_TOP_MY < nroots ? DEEPEN_TOP_MY : nroots;
    int deep_done = 0;
    ...
    for (int i = 1; i < nroots; i++)
        if (roots[i].worst > roots[best_ri].worst ||
            (roots[i].worst == roots[best_ri].worst && roots[i].mean > roots[best_ri].mean))
            best_ri = i;
